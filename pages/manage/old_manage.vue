  ...
  <view class="box">
    ...
    <view class="owner" v-if="isAdmin=='Y'">
      ...
      <view class="tab_main" v-show="currenttab==0">
        ...
        <view class="main_one" v-for="item in alllist" :key='item.id' @click.stop="getdetail(item.id)">
          <view class="cir">
          ...
          <view class="doc_name">
            <view style="display: inline-block; width: 240rpx;">健康专员：{{item.employeeName}}
              <view :class="item.is_top==0?'settop':'deltop'" @click.stop="settop(item.is_top,item.id)">{{item.is_top==0?'置顶':'取消置顶'}}</view>
            </view>
          </view>
          <view class="point">
  ...
  data() {
    return {
      ...
      alllist: [
        ...
        // customer_header: ""
        // employeeName: "内部测试店长"
        // id: "8dcb34ce-4b29-4571-9f64-639d407b7202"
        // is_top: 0
        // phone: "18666666666"
        // }
      ], //所有客户
