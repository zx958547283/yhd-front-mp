		...
		<view class="home_list">
			<view class="pressure" style="margin-bottom: 34rpx;">
				...
				<view class="main">
					...
					<view class="main_img" @click.stop="order">
						<image src="../../static/home/khyy.png" mode=""></image>
						<view class="main_text">
							客户预约
						</view>
					</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			order(){
				uni.navigateTo({
					url:'/pages/manage/timeTable'
				})			   
			},
			skill(){
				uni.navigateTo({url:'/pages/learn/skill?def_list_id='+this.learn[2].def_list_id+ '&name='+this.learn[2].name})
			},
			share(){
				uni.navigateTo({url:'/pages/learn/share?def_list_id='+this.learn[1].def_list_id+ '&name='+this.learn[1].name})
			},
			basic(){
				uni.navigateTo({url:'/pages/learn/basic?def_list_id='+this.learn[0].def_list_id+ '&name='+this.learn[0].name})
			},
			// 获取首页轮播图/学习中心
			gethome(){
