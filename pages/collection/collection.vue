			...
			<view class="box-style">
				...
				<uni-list>
					<uni-list-item title="" note="" v-for="(item,index) in pressure" :key='index'>
						<view class="point">
							<i :class="{hide:index!=0||pressure_his!=''}"></i>
						</view>
						<view class="title">
							<view :class="{textHide:index!=0||pressure_his!=''}" style="display: inline-block;font-weight: 700;">血压(mmHg)</view>
						</view>
						<view class="write">
			...
			<view class="box-style">
				...
				<uni-list>
					<uni-list-item title="" note="" v-for="(item,index) in oxygen" :key='index'>
						<view class="point">
							<i :class="{hide:index!=0||oxygen_his!=''}"></i>
						</view>
						<view class="title">
							<view :class="{textHide:index!=0||oxygen_his!=''}" style="font-weight: 700;">血氧(%)</view>
						</view>
						<view>
							<input type="digit" value="" placeholder="请输入" v-model="item.data" />
			...
			<view class="box-style">
				...
				<uni-list>
					<uni-list-item title="" note="" v-for="(item,index) in sugar" :key='index'>
						<view class="point">
							<i :class="{hide:index!=0||sugar_his!=''}"></i>
						</view>
						<view class="title">
							<view :class="{textHide:index!=0||sugar_his!=''}" style="font-weight: 700;">血糖(mmol/L)</view>
						</view>
						<view>
							<input type="digit" value="" placeholder="请输入" v-model="item.data" />
	...
	export default {
		...
		methods: {
			...
			clickSave() {
				...
				var detail = [...this.pressure_arr, ...this.oxygen_arr, ...this.sugar_arr]
				if (detail.length == 0) {
					uni.showToast({
						title: '请添加采集数据',
						icon: 'none'
				...
				pakTool.request(this,'/madyApp/saveMemberMonitoring',requestPak).then(res=>{
					const {data,code}= res
					if(code==200){
						var that = this
						uni.showToast({
			...
			gethis() {
				...
				pakTool.request(this,'/madyApp/getMemberMonitoring',requestPak).then(res=>{
					const {data}= res
					this.pressure_his = data.xueyaList
					this.oxygen_his = data.xueyangList
					this.sugar_his = data.xuetangList
				})
