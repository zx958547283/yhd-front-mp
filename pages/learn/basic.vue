...
<script>
	...
	export default {
		...
		methods: {
			more(){
				uni.navigateTo({
					url:'/pages/learn/basicMore?def_list_id='+this.def_list_id+'&name='+this.name
				})
			},
			enter(item){
				if(item.is_link!=1){
					uni.navigateTo({
						url:'/pages/learn/article?detail='+ encodeURIComponent(JSON.stringify(item))
					})
				}else{
					uni.navigateTo({
						url:'/pages/learn/imgArticle?imgUrl='+encodeURIComponent(JSON.stringify(item.url))})
				}
			},
			getlist(){
				...
				pakTool.request(this,'/content/getContentRemByDefListId',requestPak).then(res=>{
					const {data}= res 
					data.forEach(item=>{
						if(item.tags){
